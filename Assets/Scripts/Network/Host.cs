...
public class Host : Common
{
    ...
    new void Start()
    {
        ...
        //localAvatar.GetComponent<CustomAvatar>().SetContext( context.GetComponent<Context>() );
    ...
    void GetLoggedInUserCallback( Message<User> msg )
    {
        if ( !msg.IsError )
        {
            ...
            Matchmaking.CreateAndEnqueueRoom2("piootabouret", matchmakingOptions ).OnComplete( CreateAndEnqueueRoomCallback );
        }
        else
        {
    ...
    //{
    //    var avatar = localAvatar.GetComponent<CustomAvatar>();
    ...
    //            localAvatar.GetComponent<CustomAvatar>().GetAvatarState( out avatarState[numAvatarStates] );
